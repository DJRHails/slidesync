...
def render_equation(source: str, color: str = INK_HEX) -> Path | None:
    ...
    source) and reused on the next call with the same inputs. Any failure —
    matplotlib missing, or a construct outside the mathtext subset — is logged
    as a warning and surfaced as `None`.
    """
    digest = hashlib.sha1(f"{color}\n{source}".encode("utf-8")).hexdigest()
    out = EQUATION_CACHE_DIR / f"{digest}.png"
    if out.exists():
        return out
    png = _render_mathtext(source, color)
    if png is None:
        return None
...
def _render_mathtext(source: str, color: str) -> bytes | None:
    ...
    try:
        fig.text(x=0, y=0, s=f"${tex}$", fontsize=EQUATION_PT, color=color)
        buf = io.BytesIO()
